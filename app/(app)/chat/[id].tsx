...
export default function MessagesScreen() {
    ...
    const loadMessages = async () => {
        ...
        try {
            // Cargar mensajes de texto entre los dos usuarios
            ...
            // Scroll al final SIEMPRE en la primera carga
            setTimeout(() => {
            ...
            // Marcar imágenes como vistas
            const unviewedImageIds = (imagesData || [])
                .filter(img => img.to_user_id === user.id && !img.viewed)
                .map(img => img.id);

            if (unviewedImageIds.length > 0) {
                await supabase
                    .from('images_private')
                    .update({ viewed: true })
                    .in('id', unviewedImageIds);
            }

            // Marcar notas de voz como escuchadas
            const unlistenedVoiceIds = (voiceData || [])
                .filter(voice => voice.to_user_id === user.id && !voice.listened)
                .map(voice => voice.id);

            if (unlistenedVoiceIds.length > 0) {
                await supabase
                    .from('voice_notes')
                    .update({ listened: true })
                    .in('id', unlistenedVoiceIds);
            }
        } catch (error) {
