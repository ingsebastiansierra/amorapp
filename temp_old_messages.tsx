...
export default function MessagesScreen() {
    ...
    // Funci├│n para reproducir sonido de env├¡o
    ...
    useEffect(() => {
        ...
        updateLastSeen(); // Registrar que el usuario entr├│ al chat
        loadBackground(); // Cargar fondo del chat

        // Polling para actualizar la emoci├│n cada 3 segundos
        const emotionInterval = setInterval(loadMyCurrentEmotion, 3000);

        // Actualizar last_seen cada 30 segundos mientras est├í en el chat
    ...
    useEffect(() => {
        if (user) {
            ...
            // Suscripci├│n en tiempo real para mensajes nuevos
            const channel = supabase
                ...
                .on(
                    ...
                    (payload) => {
                        console.log('­ƒô¿ Nuevo mensaje recibido en tiempo real:', payload);
                ...
                .on(
                    ...
                    (payload) => {
                        console.log('­ƒôØ Mensaje actualizado en tiempo real:', payload);
                        // Recargar mensajes cuando se actualiza uno (ej: marcado como le├¡do)
                        loadMessages();
                    }
                )
                .subscribe();
    ...
    const formatLastSeen = (lastSeenDate: string | null) => {
        if (!lastSeenDate) return 'Sin conexi├│n';
        ...
        // Considerar en l├¡nea si la ├║ltima actualizaci├│n fue hace menos de 2 minutos
        if (diffMins < 2) return 'En l├¡nea';
        ...
        if (isToday) {
            return `├Ült. vez hoy a las ${timeStr}`;
        } else if (isYesterday) {
            return `├Ült. vez ayer a las ${timeStr}`;
        } else {
            // Fecha completa para d├¡as anteriores
            const dateStr = date.toLocaleDateString('es-ES', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric'
            });
            return `├Ült. vez ${dateStr} a las ${timeStr}`;
        }
    };
    ...
    const loadMessages = async () => {
        ...
        try {
            ...
            // Cargar im├ígenes privadas como mensajes
            ...
            // Combinar mensajes, im├ígenes y notas de voz
            ...
            const imageMessages: Message[] = (imagesData || []).map(img => ({
                id: img.id,
                message: img.caption || '­ƒôÀ Imagen privada',
                synced_emotion: 'happy', // Emoci├│n por defecto para im├ígenes
            ...
            const voiceMessages: Message[] = (voiceData || []).map(voice => ({
                id: voice.id,
                message: '­ƒÄñ Nota de voz',
                synced_emotion: 'happy', // Emoci├│n por defecto para voz
            ...
            // Scroll al final solo si la bandera est├í activa (primera carga o despu├®s de enviar)
            if (shouldScrollToEnd.current) {
                setTimeout(() => {
                    scrollViewRef.current?.scrollToEnd({ animated: false });
                }, 100);
                shouldScrollToEnd.current = false; // Desactivar despu├®s del primer scroll
            }

            // Marcar como le├¡dos los mensajes de texto recibidos
    ...
    const handleSendMessage = async () => {
        ...
        // Activar scroll autom├ítico para este mensaje
        ...
        try {
            ...
            if (error) {
                console.error('ÔØî Error enviando mensaje:', error);
                // Remover mensaje optimista si falla
                setMessages(prev => prev.filter(m => m.id !== optimisticMessage.id));

                // Mostrar error espec├¡fico al usuario
                if (error.message.includes('Demasiados mensajes')) {
                    Alert.alert('Espera un momento', 'Est├ís enviando mensajes muy r├ípido. Espera unos segundos.');
                } else if (error.message.includes('message_length')) {
                    Alert.alert('Mensaje muy largo', 'El mensaje no puede tener m├ís de 500 caracteres.');
                } else {
            ...
            // Enviar notificaci├│n push a la pareja
            if (partner.push_token && userData.name) {
                try {
                    ...
                    console.error('ÔÜá´©Å Error enviando notificaci├│n (mensaje ya guardado):', notifError);
                    // No fallar el env├¡o del mensaje si solo falla la notificaci├│n
                }
            }
        } catch (error) {
            console.error('ÔØî Error general en handleSendMessage:', error);
    ...
    const handleOpenImage = async (imageId: string) => {
        try {
            ...
            if (imageData.is_expired) {
                Alert.alert('No disponible', 'Esta imagen ya no est├í disponible');
                return;
            }
    ...
    const handleOpenPartnerProfile = async () => {
        ...
        // Cargar fotos p├║blicas de la pareja
        try {
    ...
    const handleScrollToMessage = (messageId: string) => {
        ...
        if (messageIndex === -1) {
            Alert.alert('Mensaje no encontrado', 'El mensaje citado ya no est├í disponible.');
            return;
        }
        ...
        if (messageRef) {
            messageRef.measureLayout(
                ...
                (x: number, y: number) => {
                    // Scroll con un poco m├ís de espacio arriba para mejor visibilidad
                    scrollViewRef.current?.scrollTo({ y: Math.max(0, y - 150), animated: true });

                    // Resaltar el mensaje temporalmente
                    setHighlightedMessageId(messageId);

                    // Feedback h├íptico m├ís notorio
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);

                    // Quitar el resaltado despu├®s de 3 segundos (aumentado de 2)
                    setTimeout(() => {
                        setHighlightedMessageId(null);
                    }, 3000);
                },
                () => {
                    // Error en measureLayout - intentar scroll b├ísico
                    console.log('Error en measureLayout, intentando scroll b├ísico');
                        ...
                        >
                            <View style={styles.messageHeader}>
                                <Text style={styles.messageEmoji}>
                                    {isImage ? '­ƒôÀ' : isVoice ? '­ƒÄñ' : (emotionConfig?.emoji || '­ƒÆò')}
                                </Text>
                                ...
                                >
                                    ...
                                    <View style={styles.quotedMessageContent}>
                                        <Text style={[
                                            ...
                                            {msg.replied_message.from_user_id === user?.id ? 'T├║' : partner?.name || 'Pareja'}
                                        </Text>
                                        <Text
                                            style={[
    ...
    // Obtener el color de fondo basado en la emoci├│n actual
        ...
        <SafeAreaView style={[styles.container, { backgroundColor }]} edges={['top']}>
            ...
            >
                            ...
                            <View style={styles.headerCenter}>
                                ...
                                >
                                    ...
                                    <Text style={styles.headerSubtitle} numberOfLines={1} ellipsizeMode="tail">
                                        {partner?.last_seen ? formatLastSeen(partner.last_seen) : 'Sin conexi├│n'}
                                    </Text>
                                </Pressable>
                            </View>
                    ...
                    >
                        {Object.keys(groupedMessages).length === 0 ? (
                            <View style={styles.emptyState}>
                                <Text style={styles.emptyEmoji}>­ƒÆî</Text>
                                <Text style={styles.emptyText}>No hay mensajes a├║n</Text>
                                <Text style={styles.emptySubtext}>Inicia una conversaci├│n con tu pareja</Text>
                            </View>
                        ) : (
                ...
                <View style={styles.inputContainerWrapper}>
                    {/* Preview del mensaje al que se est├í respondiendo */}
                    {replyingTo && (
                            ...
                            <View style={styles.replyPreviewContent}>
                                ...
                                <View style={styles.replyPreviewText}>
                                    <Text style={styles.replyPreviewName}>
                                        {replyingTo.from_user_id === user?.id ? 'T├║' : partner?.name || 'Pareja'}
                                    </Text>
                                    <Text style={styles.replyPreviewMessage} numberOfLines={1}>
                                        {replyingTo.type === 'voice' ? '­ƒÄñ Nota de voz' :
                                            replyingTo.type === 'image' ? '­ƒôÀ Imagen' :
                                                replyingTo.message}
                                    </Text>
                                </View>
                            </View>
                    ...
                    <View style={styles.inputContainer}>
                        ...
                        {/* Mostrar bot├│n de voz cuando no hay texto, bot├│n de enviar cuando hay texto */}
                        {!newMessage.trim() && partner ? (
                            <View style={styles.voiceButtonWrapper}>
                                <VoiceRecorderButton
                                    ...
                                    onSent={() => {
                                        // Recargar mensajes cuando realmente se env├¡e
                                        loadMessages();
                                    }}
                                    size={24}
                                    color="#EB477E"
                        ...
                        {viewingImage && (
                            <>
                                ...
                                <Pressable style={styles.closeImageButton} onPress={handleCloseImage}>
                                    <Text style={styles.closeImageButtonText}>Ô£ò</Text>
                                </Pressable>

                                <View style={styles.warningOverlay}>
                                    <Text style={styles.warningText}>
                                        ­ƒöÑ Esta imagen se autodestruir├í al cerrar
                                    </Text>
                                </View>

                                <View style={styles.protectionOverlay}>
                                    <Text style={styles.protectionText}>­ƒöÆ Capturas bloqueadas</Text>
                                </View>
                            </>
                        )}
                    </View>
                        ...
                        >
                            ...
                            {partnerPhotos.length > 0 && (
                                <>
                                    <View style={styles.partnerPhotosHeader}>
                                        ...
                                        <Text style={styles.partnerPhotosTitle}>Fotos p├║blicas</Text>
                                    </View>
                            ...
                            {partnerPhotos.length === 0 && (
                                <View style={styles.noPhotosContainer}>
                                    ...
                                    <Text style={styles.noPhotosText}>No hay fotos p├║blicas</Text>
                                </View>
                            )}
